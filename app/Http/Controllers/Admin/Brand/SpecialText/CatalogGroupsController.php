...
class CatalogGroupsController extends BrandLevelController
{
    private $group;
     ...
     */
    public function edit(Request $request, Company $company, Brand $brand, int $group)
    {
        return VistasGafaFit::view('admin.company.special-texts.catalog-groups.form', [
            'groupCatalog' => $this->group,
            'form_action'  => route('admin.company.brand.special-text.group.save', [
                'company' => $company,
                'brand'   => $brand,
                'group'   => $group,
            ]),
     ...
     */
    public function save(Request $request, Company $company, Brand $brand, int $group)
    {
     ...
     * @return \Illuminate\Contracts\View\Factory|\Illuminate\View\View
     */
    public function new(Request $request, Company $company, Brand $brand)
    {
        $catalogs_id = $request->get('catalogs_id');
        $brands_id = $request->get('brands_id');

        return VistasGafaFit::view('admin.company.special-texts.catalog-groups.form', [
            'form_action' => route('admin.company.brand.special-text.group.save.new', [
                'company' => $company,
                'brand'   => $brand,
            ]),
