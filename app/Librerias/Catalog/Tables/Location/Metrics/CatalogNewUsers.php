...
//        $name->setGetValueNameFilter(function () use ($user) {
...
//        $staff->setGetValueNameFilter(function () use ($user) {
//            $staff = $user->reservations->first()->staff;
//
//            return $staff ? $staff->name . ' ' . $staff->lastname : '--';
//        });
    ...
    {
        return VistasGafaFit::view('admin.location.metrics.users.filters.new-users');
    }
...
//        $res_group = Reservation::select(DB::raw('min(meeting_start) min_start,min(id) min_id,user_profiles_id profile_id'))
//            ->where('locations_id', $locations_id)
//            ->where('cancelled', false)
//            ->groupBy('user_profiles_id');
//        $res_group_sql = $res_group->toSql();
//        foreach ($res_group->getBindings() as $binding) {
//            $value = is_numeric($binding) ? $binding : "'" . $binding . "'";
//            $res_group_sql = preg_replace('/\?/', $value, $res_group_sql, 1);
//        }
//        $reservations = Reservation::join(DB::raw('(' . $res_group_sql . ') as a'), 'a.min_id', 'reservations.id')
//            ->select('staff.name', 'staff.lastname', 'reservations.user_profiles_id', 'reservations.staff_id as staffid')
//            ->leftJoin('staff', 'staff.id', 'reservations.staff_id');

        $earliest_reservations_sql = self::getEarliestReservationsSql($locations_id);
...
//            $q->when($not_in, function ($q) use ($not_in_start, $not_in_end) {
//                $q->whereDate('meeting_start', '<', $not_in_start);
//                $q->orWhere(function ($q) use ($not_in_end) {
//                    $q->whereDate('meeting_start', '>', $not_in_end);
//                });
//            });
        ...
        $query->withCount(['reservations' => function ($q) use ($locations_id) {
            $q->where('locations_id', $locations_id);
            $q->where('cancelled', 0);
        }]);
        $query->whereHas('reservations', function ($q) use ($locations_id) {
            $q->where('locations_id', $locations_id);
            $q->where('cancelled', 0);
        });
    ...
    public function GetFooterHtml()
    {
        ...
        $query = UserProfile::select('id',
            ...
            ->withCount(['reservations' => function ($q) use ($locations_id) {
                $q->where('locations_id', $locations_id);
                $q->where('cancelled', 0);
            }])
        ...
        return VistasGafaFit::view('admin.location.metrics.users.footer', [
    ...
    private static function getEarliestReservationsSql($locations_id)
    {
