...
class CatalogTopUser extends LibCatalogoModel
{
    ...
    {
        return VistasGafaFit::view('admin.brand.metrics.users.filters.top-users');
    }
    ...
    protected static function filtrarQueries(&$query)
    {
        ...
        $query->withCount(['reservations' => function ($q) use ($brands_id, $start, $end, $not_in, $not_in_start, $not_in_end, $locations) {
        ...
        $query->whereHas('reservations', function ($q) use ($brands_id, $start, $end, $not_in, $not_in_start, $not_in_end,$locations) {
            $q->where('brands_id', $brands_id);
            $q->where('cancelled', 0);
