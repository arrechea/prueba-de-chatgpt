...
initializeDeploymentConfig() {
    ...
    COMPOSER_ARGS="--no-interaction --prefer-dist  --optimize-autoloader --no-progress --verbose --no-dev"
...
if [ -e "$DEPLOYMENT_TARGET/artisan" ]; then
echo "Found Artisan"
pushd "$DEPLOYMENT_TARGET"
php artisan migrate --force
#exitWithMessageOnError "Fallo al correr migraciones"

# 5. Clear Cache,Views, config
