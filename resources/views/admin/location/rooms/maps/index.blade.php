                    ...
                    <div class="BuqSaas-c-nav is-list">
                        ...
                        <div class="BuqSaas-c-nav__add">
                            <a  class="BuqSaas-e-button is-add"
                                href="{{route('admin.company.brand.locations.room-maps.create',['company'=>$company,'brand'=>$brand,'location'=>$location])}}">
                                <i class="material-icons small">add</i>{{__('common.add')}}
                            </a>
                        </div>
                    </div>
                </div>
                <div class="BuqSaas-l-list__body is-locationRoomsMaps">
                    @include('admin.catalog.table')
                </div>
            </div>
        @else
            @include('admin.location.rooms.maps.tabs')
            <div class="list__container">
                <div class="row">
                    <div class="card-panel radius--forms">
                        <h5 class="header">{{__('maps.ListMaps')}}</h5>
                        @include('admin.catalog.table')
                    </div>
                </div>
            </div>
