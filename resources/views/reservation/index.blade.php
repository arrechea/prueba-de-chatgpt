...
<div id="CreateReservationFancy--Block">
    ...
    <div id="CreateReservationFancy--Back"></div>
    <script>
        //        Paypal
        if (!window.paypal) {
            (function () {
        ...
        if ($) {
            $('#CreateReservationFancy--Back').on('click', function () {
                $('#CreateReservationFancy--Block').remove();
            })
        }
    </script>
    @include('window.credit-cards')
    <script src="{{mixGafaFit('/js/admin/react/reservation/process/build.js')}}"></script>
</div>
@if($isExternalApp)
</body>
</html>
